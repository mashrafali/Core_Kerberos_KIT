...
#Enough introduction now lets talk buisness' >> 1-BIN/worker.sh
while read -r line
do
     echo -en 'send -- {' >> 1-BIN/worker.sh; echo -en $line >> 1-BIN/worker.sh; echo '}' >> 1-BIN/worker.sh    ## dont mess with BELOW SPACING !!
     echo 'send -- "\r"
expect "#"
send -- "\r"
expect "#"' >> 1-BIN/worker.sh

done < "$filename2"
echo  >> 1-BIN/worker.sh
echo '#Waving goodbye
send -- "exit\r"' >> 1-BIN/worker.sh
...
do
     ...
     echo "########################################################################" >> $logging
     echo >> $logging
     ./1-BIN/worker.sh $device $myidentity $myprecious >> $logging 2>&1
     echo >> $logging
     echo >> $logging
     cat $logging
